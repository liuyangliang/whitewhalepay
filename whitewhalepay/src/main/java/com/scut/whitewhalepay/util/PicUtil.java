...
public class PicUtil {
	...
	public static String saveTransactionPic(File srcPic, String srcPicName) throws Exception{
		//需要自己定义图片保存路径
		String path = "D:/Web/whitewhalepay/src/main/webapp/img/Transaction/"+new SimpleDateFormat("YYYY/MM/dd").format(new Date())+"/";
		File pathFile = new File(path);
		if (!pathFile.exists()) pathFile.mkdirs();
		String destPicName = UUID.randomUUID().toString() + srcPicName.substring(srcPicName.lastIndexOf("."));
		File destPic = new File(path + destPicName);
